...
output_directory <-
  paste("rnaseq", "process", "cuffdiff", argument_list$comparison_name, sep = "_")
...
cuff_set <-
  readCufflinks(
    dir = cuffdiff_directory, gtfFile = argument_list$gtf_file, genome = argument_list$genome_version
  )
...
if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
  ...
  write.table(
    x = runInfo(object = cuff_set), file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
  )
}
rm(frame_path)
...
if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
  ...
  write.table(
    x = sample_frame, file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
  )
}
rm(frame_path)
...
if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
  ...
  write.table(
    x = aperm(a = sample_pairs), file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
  )
}
...
if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
  ...
  write.table(
    x = replicate_frame, file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
  )
}
rm(frame_path)
...
if (file.exists(plot_path_pdf) &&
  ...
  ggsave(
    filename = plot_path_pdf, plot = ggplot_object, width = plot_width, height = argument_list$plot_height
  )
  ggsave(
    filename = plot_path_png, plot = ggplot_object, width = plot_width, height = argument_list$plot_height
  )
...
if (file.exists(plot_path_pdf) &&
  ...
  ggsave(
    filename = plot_path_pdf, plot = ggplot_object, width = argument_list$plot_width, height = argument_list$plot_height
  )
  ggsave(
    filename = plot_path_png, plot = ggplot_object, width = argument_list$plot_width, height = argument_list$plot_height
  )
  rm(ggplot_object)
}
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_genes_density_wo_replicates.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_genes_density_wo_replicates.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_genes_density_w_replicates.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_genes_density_w_replicates.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_isoforms_density_wo_replicates.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_isoforms_density_wo_replicates.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_isoforms_density_w_replicates.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_isoforms_density_w_replicates.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_genes_box_w_replicates.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_genes_box_w_replicates.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
  ...
  ggplot_object <-
    ggplot_object + geom_boxplot(
      mapping = aes(
        x = condition, y = log10(fpkm), fill = condition
      ), size = 0.3, alpha = I(1 / 3)
    )
  ggplot_object <-
    ggplot_object + theme(axis.text.x = element_text(angle = -90, hjust = 0))
  ggplot_object <-
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_genes_box_wo_replicates.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_genes_box_wo_replicates.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_isoforms_box_w_replicates.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_isoforms_box_w_replicates.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_isoforms_box_wo_replicates.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_isoforms_box_wo_replicates.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
if (sample_number <= 20) {
  plot_path_pdf <-
    file.path(output_directory, paste0(prefix, "_genes_scatter_matrix.pdf"))
  plot_path_png <-
    file.path(output_directory, paste0(prefix, "_genes_scatter_matrix.png"))
  if (file.exists(plot_path_pdf) &&
      (file.info(plot_path_pdf)$size > 0) &&
      file.exists(plot_path_png) &&
      (file.info(plot_path_png)$size > 0)) {
...
if (sample_number <= 20) {
  plot_path_pdf <-
    file.path(output_directory, paste0(prefix, "_isoforms_scatter_matrix.pdf"))
  plot_path_png <-
    file.path(output_directory, paste0(prefix, "_isoforms_scatter_matrix.png"))
  if (file.exists(plot_path_pdf) &&
      (file.info(plot_path_pdf)$size > 0) &&
      file.exists(plot_path_png) &&
      (file.info(plot_path_png)$size > 0)) {
  ...
  plot_path_pdf <-
    file.path(
      output_directory, paste(
        prefix, sample_pairs[1, i], sample_pairs[2, i], "genes_scatter.pdf", sep = "_"
      )
    )
  plot_path_png <-
    file.path(
      output_directory, paste(
        prefix, sample_pairs[1, i], sample_pairs[2, i], "genes_scatter.png", sep = "_"
      )
    )
  if (file.exists(plot_path_pdf) &&
      (file.info(plot_path_pdf)$size > 0) &&
    ...
    message(
      paste(
        "Skipping a Scatter Plot on Genes for", sample_pairs[1, i], "versus", sample_pairs[2, i]
      )
    )
  } else {
    message(
      paste(
        "Creating a Scatter Plot on Genes for", sample_pairs[1, i], "versus", sample_pairs[2, i]
      )
    )
    ggplot_object <-
...
for (i in 1:length(sample_pairs[1,])) {
  plot_path_pdf <-
    file.path(
      output_directory, paste(prefix, sample_pairs[1, i], sample_pairs[2, i], "maplot.pdf", sep = "_")
    )
  plot_path_png <-
    file.path(
      output_directory, paste(prefix, sample_pairs[1, i], sample_pairs[2, i], "maplot.png", sep = "_")
    )
  if (file.exists(plot_path_pdf) &&
      (file.info(plot_path_pdf)$size > 0) &&
      file.exists(plot_path_png) &&
      (file.info(plot_path_png)$size > 0)) {
    message(paste(
      "Skipping a MAplot on Genes for", sample_pairs[1, i], "versus", sample_pairs[2, i]
    ))
  } else {
    message(paste(
      "Creating a MAplot on Genes for", sample_pairs[1, i], "versus", sample_pairs[2, i]
    ))
    ggplot_object <-
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_genes_volcano_matrix.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_genes_volcano_matrix.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
for (i in 1:length(sample_pairs[1,])) {
  plot_path_pdf <-
    file.path(
      output_directory, paste(
        prefix, sample_pairs[1, i], sample_pairs[2, i], "genes_volcano.pdf", sep = "_"
      )
    )
  plot_path_png <-
    file.path(
      output_directory, paste(
        prefix, sample_pairs[1, i], sample_pairs[2, i], "genes_volcano.png", sep = "_"
      )
    )
  if (file.exists(plot_path_pdf) &&
      (file.info(plot_path_pdf)$size > 0) &&
    ...
    message(
      paste(
        "Skipping a Volcano Plot on Genes for", sample_pairs[1, i], "versus", sample_pairs[2, i]
      )
    )
  } else {
    message(
      paste(
        "Creating a Volcano Plot on Genes for", sample_pairs[1, i], "versus", sample_pairs[2, i]
      )
    )
    ggplot_object <-
...
if (file.exists(plot_path_pdf) &&
  ...
  if (replicate_number <= 24) {
    ...
    gene_rep_res <-
      data.frame(
        names = rownames(gene_rep_fit$points), M1 = gene_rep_fit$points[,1], M2 = gene_rep_fit$points[,2]
      )
    ...
    if (replicate_number <= 24) {
      ...
      ggplot_object <-
        ggplot_object + geom_text(mapping = aes(
          x = M1, y = M2, label = names, color = names, size = 4
        ))
    }
    # Arrange a maximum of 24 replicates in each guide column.
    ggplot_object <-
...
if (file.exists(plot_path_pdf) &&
  ...
  ggplot_object <-
    PCAplot(
      object = genes(object = cuff_set), x = "PC1", y = "PC2", replicates = TRUE
    )
  ggsave(
    filename = plot_path_pdf, plot = ggplot_object, width = argument_list$plot_width, height = argument_list$plot_height
  )
...
for (i in 1:length(sample_pairs[1,])) {
  frame_path <-
    file.path(
      output_directory, paste(prefix, sample_pairs[1, i], sample_pairs[2, i], "genes_diff.tsv", sep = "_")
    )
  if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
    message(
      paste(
        "Skipping a differential data frame on Genes for", sample_pairs[1, i], "versus", sample_pairs[2, i]
      )
    )
  } else {
    message(
      paste(
        "Creating a differential data frame on Genes for", sample_pairs[1, i], "versus", sample_pairs[2, i]
      )
    )
    ...
    gene_diff <-
      diffData(
        object = genes(object = cuff_set), x = sample_pairs[1, i], y = sample_pairs[2, i], features = FALSE
      )
    gene_diff <- gene_diff[,-1]
    gene_merge <-
      merge(
        x = gene_frame, y = gene_diff, by.x = "gene_id", by.y = "gene_id", all = TRUE, sort = TRUE
      )
    write.table(
      x = gene_merge, file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
    )
...
for (i in 1:length(sample_pairs[1,])) {
  frame_path <-
    file.path(
      output_directory, paste(
        prefix, sample_pairs[1, i], sample_pairs[2, i], "isoforms_diff.tsv", sep = "_"
      )
    )
  if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
    message(
      paste(
        "Skipping a differential data frame on Isoforms for", sample_pairs[1, i], "versus", sample_pairs[2, i]
      )
    )
  } else {
    message(
      paste(
        "Creating a differential data frame on Isoforms for", sample_pairs[1, i], "versus", sample_pairs[2, i]
      )
    )
    # The diffData function allows automatic merging with feature annotation, but that includes some empty columns.
    ...
    isoform_diff <-
      diffData(
        object = isoforms(object = cuff_set), x = sample_pairs[1, i], y = sample_pairs[2, i], features = FALSE
      )
    isoform_diff <- isoform_diff[,-1]
    isoform_merge <-
      merge(
        x = isoform_frame, y = isoform_diff, by.x = "isoform_id", by.y = "isoform_id", all = TRUE, sort = TRUE
      )
    write.table(
      x = isoform_merge, file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
    )
...
frame_path <-
  file.path(output_directory, paste0(prefix, "_genes_fpkm_replicates.tsv"))
if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
  ...
  gene_merge <-
    merge(
      x = gene_frame, y = gene_rep_fpkm_matrix, by.x = "gene_id", by.y = "row.names", all = TRUE, sort = TRUE
    )
  write.table(
    x = gene_merge, file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
  )
...
frame_path <-
  file.path(output_directory, paste0(prefix, "_genes_counts_replicates.tsv"))
if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
  ...
  gene_merge <-
    merge(
      x = gene_frame, y = gene_rep_count_matrix, by.x = "gene_id", by.y = "row.names", all = TRUE, sort = TRUE
    )
  write.table(
    x = gene_merge, file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
  )
...
frame_path <-
  file.path(output_directory, paste0(prefix, "_isoforms_fpkm_replicates.tsv"))
if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
  ...
  isoform_merge <-
    merge(
      x = isoform_frame, y = isoform_rep_fpkm_matrix, by.x = "isoform_id", by.y = "row.names", all = TRUE, sort = TRUE
    )
  write.table(
    x = isoform_merge, file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
  )
...
frame_path <-
  file.path(output_directory, paste0(prefix, "_isoforms_counts_replicates.tsv"))
if (file.exists(frame_path) && file.info(frame_path)$size > 0) {
  ...
  isoform_merge <-
    merge(
      x = isoform_frame, y = isoform_rep_count_matrix, by.x = "isoform_id", by.y = "row.names", all = TRUE, sort = TRUE
    )
  write.table(
    x = isoform_merge, file = frame_path, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE
  )
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_genes_significance_matrix.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_genes_significance_matrix.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
plot_path_pdf <-
  file.path(output_directory, paste0(prefix, "_isoforms_significance_matrix.pdf"))
plot_path_png <-
  file.path(output_directory, paste0(prefix, "_isoforms_significance_matrix.png"))
if (file.exists(plot_path_pdf) &&
    (file.info(plot_path_pdf)$size > 0) &&
    file.exists(plot_path_png) &&
    (file.info(plot_path_png)$size > 0)) {
...
rm(
  cuff_set, output_directory, cuffdiff_directory, sample_pairs, sample_number, replicate_number, have_replicates, prefix, i,
  argument_list
)
message("All done")
