#! /usr/bin/env Rscript
#
# This script merges data tables of the Cufflinks RNA-seq analysis stage after
# bsf_rnaseq_process_cufflinks.R to come up with consistent tables for genes
# (rnaseq_cufflinks_isoforms_fpkm_tracking.tsv) and isoforms
# (rnaseq_cufflinks_isoforms_fpkm_tracking.tsv).
#
...
process_cufflinks_table <- function(replicate_name, object_type, merge_frame = NULL) {
  ...
  file_path <- file.path(prefix_cufflinks, paste(prefix_cufflinks, object_type, "fpkm_tracking.tsv", sep = "_"))
  cufflinks_frame <- read.table(file = file_path, header = TRUE, stringsAsFactors = FALSE)
  rm(file_path)
  ...
  subset_frame <- cufflinks_frame[, !(colnames(cufflinks_frame) %in% obsolete_columns)]
  rm(cufflinks_frame, obsolete_columns)
  
  # Select only Ensembl objects i.e. those that have a gene_biotype set.
  ensembl_frame <- subset_frame[! is.na(x = subset_frame$gene_biotype), ]
  rm(subset_frame)
  
  # Rename columns to include the replicate name.
  if (object_type == "isoforms") {
    colnames(ensembl_frame)[grepl('^coverage$', colnames(ensembl_frame))] <- paste('coverage', replicate_name, sep = '_')
  }
  colnames(ensembl_frame)[grepl('^FPKM$', colnames(ensembl_frame))] <- paste('FPKM', replicate_name, sep = '_')
  colnames(ensembl_frame)[grepl('^FPKM_conf_lo$', colnames(ensembl_frame))] <- paste('FPKM_conf_lo', replicate_name, sep = '_')
  colnames(ensembl_frame)[grepl('^FPKM_conf_hi$', colnames(ensembl_frame))] <- paste('FPKM_conf_hi', replicate_name, sep = '_')
  colnames(ensembl_frame)[grepl('^FPKM_status$', colnames(ensembl_frame))] <- paste('FPKM_status', replicate_name, sep = '_')
  ...
  if (is.null(x = merge_frame)) {
    ...
    return(merge(x = merge_frame, y = ensembl_frame, all = TRUE, sort = TRUE))
  }
}
...
write.table(x = merge_frame, file = file_path, col.names = TRUE, row.names = FALSE, sep = "\t")
...
rm(replicate_names, process_cufflinks_table, opt, option_list)
